...
func to_dict() -> Dictionary:
	var dict : Dictionary = {"Nodes" : {}, "StartNodes" : {}, "Connections" : node_connections.duplicate(true)}
	
	for node_name in node_list.keys():
		var node = node_list.get_value(node_name)
		if node is EditorNodeInfo:
			dict["Nodes"][node_name] = node.to_dict()
	
	for start_node in start_nodes.keys():
		dict["StartNodes"][start_node] = start_nodes.get_value(start_node)
