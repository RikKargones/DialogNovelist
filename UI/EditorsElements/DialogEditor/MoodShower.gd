...
var mood_variants_list 		: DialogDispetcher.UnicDict = DialogDispetcher.UnicDict.new()
...
func update_shower() -> void:
	...
	if blockinfo is DialogDispetcher.NBI_Mood:
		...
		match blockinfo.align:
			DialogDispetcher.PersonProfile.ALIGN_LIST.LEFT:
				picture_aspect.alignment_horizontal = AspectRatioContainer.ALIGN_BEGIN
			DialogDispetcher.PersonProfile.ALIGN_LIST.CENTER:
				picture_aspect.alignment_horizontal = AspectRatioContainer.ALIGN_CENTER
			DialogDispetcher.PersonProfile.ALIGN_LIST.RIGHT:
