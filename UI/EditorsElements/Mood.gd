...
func _on_ChangeBt_pressed() -> void:
	var popup = FilePopup.new("Pick new mood texture...", ["*.png, *.jpg, *.jpeg, *.bmp; Supported images"])
	Ui.add_popup(popup, "file_selected", self, "on_mood_new_texture_pick")


func _on_DeleteBt_pressed() -> void:
	var popup = ConfirmPopup.new("Are you sure you want to delete this mood?")
	Ui.add_popup(popup, "confirmed", self, "on_mood_delete")
